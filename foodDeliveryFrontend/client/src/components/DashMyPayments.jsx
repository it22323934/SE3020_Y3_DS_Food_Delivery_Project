...
export default function DashMyPayments() {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const res = await fetch(`api/payment/get/user/${currentUser._id}`, {});
  ...
  const handleReportDownload = async () => {
    if (selectedCriteria !== null) {
      ...
      try {
        const res = await fetch("api/payment/downloadReportStatusUser", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
