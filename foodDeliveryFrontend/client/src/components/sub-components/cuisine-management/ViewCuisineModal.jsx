    ...
    >
      <Modal.Header className="border-b border-gray-200 bg-gray-50">
        <div className="flex items-center">
          ...
          <h3 className="text-xl font-bold text-gray-900">Cuisine Type Details</h3>
        </div>
      </Modal.Header>
          ...
          <div className="space-y-4">
            <div className="flex flex-col items-center justify-center bg-gray-50 p-4 rounded-lg">
              <div className="w-48 h-48 mb-4">
                <img
                  src={cuisineData.iconUrl || "https://via.placeholder.com/200?text=No+Image"}
                  alt={cuisineData.name || "Cuisine"}
                  className="rounded-lg w-full h-full object-cover border-4 border-purple-100 shadow-md"
                />
              </div>
              <h2 className="text-xl font-bold text-center">{cuisineData.name}</h2>
              <Badge
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="font-semibold text-gray-700 flex items-center mb-2">
                <HiOutlineInformationCircle className="mr-2 text-purple-500" /> Basic Info
              </h3>
              <div className="space-y-2 text-sm">
                <div>
                  <span className="text-gray-500">ID:</span>
                  <span className="ml-2 font-medium">{cuisineData.id || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-gray-500">Created:</span>
                  <span className="ml-2 font-medium">{formatDate(cuisineData.createdAt)}</span>
                </div>
                <div>
                  <span className="text-gray-500">Last Updated:</span>
                  <span className="ml-2 font-medium">{formatDate(cuisineData.updatedAt)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 md:col-span-2">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="font-semibold text-gray-700 flex items-center mb-3">
                <FaStore className="mr-2 text-purple-500" /> Associated Restaurants
              </h3>
              
              {cuisineData?.restaurants && cuisineData?.restaurants.length > 0 ? (
                    ...
                    <Table.Body className="divide-y">
                      {cuisineData?.restaurants.map((restaurant) => (
                        <Table.Row key={restaurant.id} className="bg-white">
                          ...
                          <Table.Cell>
                            <Badge
                              color={restaurant.active ? "success" : "failure"}
                ...
                <div className="text-center py-4 text-gray-500">
                  No restaurants are currently associated with this cuisine type.
                </div>
              )}
            </div>
          </div>
