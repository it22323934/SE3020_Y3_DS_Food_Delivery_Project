...
export const UpdateCuisineModal = ({
  ...
  // Initialize form data when cuisine data changes
  useEffect(() => {
    if (cuisineData) {
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Make sure we have the cuisine ID
  ...
  const handleClose = () => {
    setFile(null);
    setImagePreview(cuisineData?.iconUrl || null);
      ...
      <Modal.Body className="px-6">
        ...
        {success && (
          <Alert color="success" className="mb-4">
            Cuisine type updated successfully!
          </Alert>
        )}
