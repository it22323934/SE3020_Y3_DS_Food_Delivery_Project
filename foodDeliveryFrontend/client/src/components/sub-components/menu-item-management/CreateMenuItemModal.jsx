...
export default function CreateMenuItemModal({
  ...
  // All the effect hooks and functions remain the same
  useEffect(() => {
    if (show) {
      setFormData({
        restaurantId,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await menuItemService.createMenuItem(submitData, token);
            ...
            <Card className="shadow-sm hover:shadow-md transition-shadow duration-300">
              ...
              <div className="border-t pt-4">
                <h4 className="text-md font-medium mb-4 text-gray-700 flex items-center gap-1">
                  <HiOutlinePlusCircle size={16} className="text-indigo-500" />
                  Add New Add-On
                </h4>
