...
import { Button, Card, Badge, Spinner, Tabs, Modal, TextInput, Tooltip, Alert } from 'flowbite-react';
import { 
  FaArrowLeft, FaStar, FaClock, FaMapMarkerAlt, FaPhoneAlt, FaUtensils, 
  FaDollarSign, FaShoppingCart, FaSearch, FaTag, FaPercent, FaAward
} from 'react-icons/fa';
...
export default function RestaurantDetail() {
  ...
  const [availablePromoCodes, setAvailablePromoCodes] = useState([
    { code: 'WELCOME10', description: '10% off your first order', discount: 10 },
    { code: 'FREESHIP', description: 'Free delivery on orders over $25', discount: 0 },
    { code: 'LUNCH20', description: '20% off on lunch specials (11AM-2PM)', discount: 20 },
  ]);
                  ...
                  {availablePromoCodes.map((promo, index) => (
                    ...
                    >
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-bold text-orange-600 dark:text-orange-400">{promo.code}</span>
...
function MenuItemsByCategorySection({ categoryId, token, searchTerm, restaurantId }) {
  ...
  if (filteredItems.length === 0) {
    return (
      <div className="text-center py-8 bg-gray-50 dark:bg-gray-800 rounded-lg">
        ...
        <p className="text-gray-500 dark:text-gray-400">No menu items match your search</p>
        {searchTerm && (
              ...
              <div className="ml-4 flex-1 flex flex-col">
                <div className="flex-grow">
                  ...
                  <div className="mt-2 font-bold flex items-center">
                    {item.onPromotion ? (
                      <>
                        ...
                        <span className="text-red-600 dark:text-red-500">${item.discountedPrice.toFixed(2)}</span>
                      </>
                    ) : (
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {item.vegetarian && <Badge color="success" size="sm">Vegetarian</Badge>}
                    {item.vegan && <Badge color="purple" size="sm">Vegan</Badge>}
                    {item.glutenFree && <Badge color="indigo" size="sm">Gluten Free</Badge>}
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap justify-between items-center">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedItem.vegetarian && <Badge color="success">Vegetarian</Badge>}
                  {selectedItem.vegan && <Badge color="purple">Vegan</Badge>}
                  {selectedItem.glutenFree && <Badge color="indigo">Gluten Free</Badge>}
              ...
              {selectedItem.preparationTimeMinutes > 0 && (
                <div className="flex items-center text-gray-600 dark:text-gray-400">
                  ...
                  <span>Preparation: {selectedItem.preparationTimeMinutes} minutes</span>
                </div>
              )}
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="space-y-2">
                    {selectedItem.addOns.map(addon => (
                      <div key={addon.id} className="flex justify-between items-center py-2 px-3 bg-gray-50 dark:bg-gray-700 rounded">
                        <div>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-500">
                            {addon.multiple ? `Max: ${addon.maxQuantity}` : 'Single'}
                          </p>
                        </div>
                      </div>
                    ))}
...
function ItemAddToCartControl({ item, onAddToCart, closeModal }) {
  ...
  const handleAddOnQuantityChange = (addonId, newQuantity) => {
    setSelectedAddOns(selectedAddOns.map(addon => 
      addon.id === addonId 
        ? { ...addon, quantity: Math.max(1, Math.min(newQuantity, addon.maxQuantity || 10)) } 
        : addon
    ));
  };
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {item.addOns.map(addon => {
              ...
              return (
                <div key={addon.id} className={`p-3 rounded border ${isSelected ? 'border-green-400 bg-green-50 dark:bg-green-900 dark:border-green-700' : 'border-gray-200 dark:border-gray-700'}`}>
                  <div className="flex justify-between items-center">
                    ...
                    {isSelected && addon.multiple && (
                      <div className="flex items-center">
                        ...
                        <span className="mx-2">{selectedAddon?.quantity || 1}</span>
                        <button 
                          onClick={() => handleAddOnQuantityChange(addon.id, (selectedAddon?.quantity || 1) + 1)}
                          className="p-1 rounded-full border"
                          disabled={(selectedAddon?.quantity || 1) >= (addon.maxQuantity || 10)}
                        >
