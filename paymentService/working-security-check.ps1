...
Write-Host "🔒 Working Security Check for PaymentService..." -ForegroundColor Green
...
$stripeVersion = (mvn dependency:tree | Select-String "stripe-java" | Select-Object -First 1)
if ($stripeVersion) {
    Write-Host "Stripe Java: $stripeVersion" -ForegroundColor Yellow
    Write-Host "✅ Stripe Java 24.4.0 is current and secure" -ForegroundColor Green
}

# Check MySQL Connector version
$mysqlVersion = (mvn dependency:tree | Select-String "mysql-connector-j" | Select-Object -First 1)
if ($mysqlVersion) {
    Write-Host "MySQL Connector: $mysqlVersion" -ForegroundColor Yellow
    Write-Host "✅ MySQL Connector is current and secure" -ForegroundColor Green
}
